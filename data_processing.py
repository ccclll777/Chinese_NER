...
class DataProcessing():
    def __init__(self):
        ...
        self.line_data = []
        self.line_label = []
        self.tag_to_id = {'': 0,'B_ns': 1, 'B_nr': 2, 'B_nt': 3,'M_nt': 4,'M_nr': 5,
                  'M_ns': 6,'E_nt': 7,'E_nr': 8, 'E_ns': 9,'o': 10}
        self.id_to_tag = {0: '',1: 'B_ns', 2: 'B_nr',3: 'B_nt',4: 'M_nt',5: 'M_nr',
                  6: 'M_ns',7: 'E_nt',8: 'E_nr',9: 'E_ns',10: 'o'}
        ...
        for line in input_data.readlines():
            ...
            for word_label_pair in line:
                ...
                for word in word_label_pair:
                    word = word.split('/')
                    data.append(word[0])
                    label.append(self.tag_to_id[word[1]])
    ...
    def X_padding(self,words):
        ...
        indexs = list(self.word_to_index[words])
        if len(indexs) >= self.max_len:  # 长则弃掉
            return indexs[:self.max_len]
        indexs.extend([0] * (self.max_len - len(indexs)))  # 短则补全
        return indexs


    def y_padding(self,indexs):
        """把 tags 转为 id 形式， 并自动补全位 max_len 长度。"""
        if len(indexs) >= self.max_len:  # 长则弃掉
            return indexs[:self.max_len]
        indexs.extend([0] * (self.max_len - len(indexs)))  # 短则补全
        return indexs
    def X_to_list(self,words):
        """把 words 转为 id 形式，并自动补全位 max_len 长度。"""
        indexs = list(self.word_to_index[words])
        return indexs


    def y_to_list(self,indexs):
        return indexs
    def get_train_valid_test_set(self):
        ...
        :return:
        """
        df_data = pd.DataFrame({'words': self.datas, 'tags': self.labels}, index=range(len(self.datas)))
        df_data['x'] = df_data['words'].apply(self.X_to_list)
        df_data['y'] = df_data['tags'].apply(self.y_to_list)
